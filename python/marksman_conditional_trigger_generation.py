...
def create_trigger_model(dataset, attack_model=None):
    ...
    if dataset == 'cifar10':
        ...
        atkmodel = ConditionalAutoencoder(10, 32)
    elif dataset == 'mnist':
        from attack_models.autoencoders import MNISTConditionalAutoencoder as Autoencoder
...
def create_models(args):
    ...
    if args.dataset == 'cifar10':
        from attack_models.autoencoders import ConditionalAutoencoder
        atkmodel = ConditionalAutoencoder(args.num_classes, args.input_height).to(args.device)
        # Copy of attack model
        tgtmodel = ConditionalAutoencoder(args.num_classes, args.input_height).to(args.device)
...
def test(args, atkmodel, scratchmodel, target_transform, 
         train_loader, test_loader, epoch, trainepoch, writer, clip_image, post_transforms=None,
    ...
    for cepoch in range(trainepoch):
        ...
        for batch_idx, (data, target) in pbar:
            ...
            with torch.no_grad():
                ...
                noise = atkmodel(data, atktarget) * args.eps
                atkdata = clip_image(data + noise)
                
                if post_transforms: #this is for training 
                    atkdata = post_transforms(atkdata)
        ...
        if cepoch % epochs_per_test == 0 or cepoch == trainepoch-1:
            ...
            with torch.no_grad():
                for data, target in test_loader:
                    ...
                    test_transform_loss += loss_fn(atkoutput, atktarget).item() * bs  # sum up batch loss
        ...
        if cepoch == 0:
            print(target[:args.test_n_size].cpu().numpy())
            print(atktarget[:args.test_n_size].cpu().numpy())
            print(atkpred[:args.test_n_size].view(-1).cpu().numpy())
...
#             data = post_transforms(data)
        ...
        data, target = data.to(args.device), target.to(args.device)
        atktarget = sample_negative_labels(target, args.num_classes).to(args.device) #randomly sample any labels
        noise = tgtmodel(data, atktarget) * args.eps
        atkdata = clip_image(data + noise)
        ...
        if args.attack_portion < 1.0:
            aug_atkdata = aug_atkdata[:int(args.attack_portion*bs)]
            atktarget = atktarget[:int(args.attack_portion*bs)]

        # Calculate loss
        atkoutput = clsmodel(aug_atkdata)
        loss_poison = loss_fn(atkoutput, atktarget)
        loss1 = loss_poison
...
def create_paths(args):
    ...
    args.checkpoint_path = checkpoint_path = os.path.join(basepath, 'checkpoint.ckpt')
    args.bestmodel_path = bestmodel_path = os.path.join(basepath, 'bestmodel.ckpt')
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % args.epochs_per_external_eval == 0 or epoch == args.epochs: 
            acc_clean, acc_poison = test(args, atkmodel, scratchmodel, target_transform, 
                   train_loader, test_loader, epoch, args.cls_test_epochs, writer, clip_image, post_transforms=post_transforms,
                   log_prefix='External')
        else:
            acc_clean, acc_poison = test(args, atkmodel, scratchmodel, target_transform, 
                   train_loader, test_loader, epoch, args.train_epoch, writer, clip_image, post_transforms=post_transforms,
                   log_prefix='Internal')
